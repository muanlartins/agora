...
const ENDPOINTS = {
  ...
  checkHashedId: (id: string, hashedId: string) =>
    `/public/member/${id}/private/${hashedId}`,
...
export class MemberService {
  ...
  public getAllMembers(): Observable<Member[]> {
    ...
    this.httpClient
      .get<Member[]>(BASE_URL + ENDPOINTS.getAllMembers)
      .subscribe((members) => {
  ...
  public async createMember(
    ...
    const member = await firstValueFrom(
      this.httpClient.post<Member>(BASE_URL + ENDPOINTS.createMember, {
        name,
        society,
        isTrainee,
        hasPfp,
  ...
  public async updateMember(
    ...
    await firstValueFrom(
      this.httpClient.put<boolean>(BASE_URL + ENDPOINTS.editMember, {
        id,
        name,
        society,
        isTrainee,
  ...
  public async uploadMemberPfp(formData: FormData) {
    await firstValueFrom(
      this.httpClient.post<boolean>(BASE_URL + ENDPOINTS.updatePfp, formData)
    );
  }

  public async deleteMember(id: string) {
    await firstValueFrom(
      this.httpClient.delete<boolean>(BASE_URL + ENDPOINTS.deleteMember(id))
    );
  ...
  public async getMember(id: string) {
    return await firstValueFrom(
      this.httpClient.get<Member>(BASE_URL + ENDPOINTS.getMember(id))
    );
  }
  ...
  public async checkHashedId(id: string, hashedId: string) {
    return await firstValueFrom(
      this.httpClient.get<boolean>(
        BASE_URL + ENDPOINTS.checkHashedId(id, hashedId)
      )
    );
  }
  ...
  public async getHashedId(id: string) {
    return await firstValueFrom(
      this.httpClient.post<string>(BASE_URL + ENDPOINTS.getHashedId(id), {})
    );
  }
}
